...
public class SearchFrame extends JFrame implements ActionListener, TableModelListener {
	...
	SearchFrame() throws Exception {
		...
		titleLabel1.setText("Enter slang word to search definition ");
		...
		JLabel formLabel = new JLabel("Enter slang word");
		...
		form.add(btnSearch, BorderLayout.LINE_END);
		...
		// Setting JFrame
		this.setTitle("Search slang word window");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnSearch) {
			...
			if (key.isEmpty()) {
				JOptionPane.showMessageDialog(this, "Please enter slang word you want to search", "Inane error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			Object[] options = { "Search Slang word", "Search Slang word's definition" };
			int n = JOptionPane.showOptionDialog(this, "Choose mode " + "you want to execute?", "Choose mode search",
			...
			if (n == 0) {
				...
				if (temp != null)
					...
					titleLabel1.setText("Can't not search that slangWord");
					return;
				}
				result = temp;
				...
				this.clearTable();
				long startTime = System.currentTimeMillis();
				temp = slangword.searchDefinition(key);
				long endTime = System.currentTimeMillis();
				long timeElapsed = endTime - startTime;
				if (temp != null)
					titleLabel1.setText("Execution time in milliseconds(" + temp.length + " Results ): "
	...
	public void tableChanged(TableModelEvent e) {
		...
		System.out.println("Row selected is: " + row + col + " : " + Data);
		if (col == 2) {
			// edit definition
			slangword.set((String) jt.getValueAt(row, 1), result[row][2], (String) jt.getValueAt(row, 2));
			JOptionPane.showMessageDialog(this, "A new row is updated.");
		}
